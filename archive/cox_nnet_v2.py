...
# This is a package for running the neural network extension of Cox regression (version 2).  
# Classes: 
...
#   signOfBeta - calculates estimated direction of variable coefficient
...
class CoxRegression(object):
    ...
    def negative_log_likelihood(self, ystatus_batch):
        return(-T.mean((self.theta - T.log(T.extra_ops.cumsum(self.exp_theta[::-1])[::-1])) * ystatus_batch)) #exp_theta * R_batch ~ sum the exp_thetas of the patients with greater time e.g., R(t)
...
class HiddenLayer(object):
    def __init__(self, rng, input, n_samples, map, label, activation=T.nnet.selu):
        ...
        for i in range(len(map)):
            W_values = numpy.asarray(
                rng.uniform(
                    low=-numpy.sqrt(6. / (int(map[i][0]) + int(map[i][2]))),
                    high=numpy.sqrt(6. / (int(map[i][0]) + int(map[i][2]))),
...
def defineSearchParams(search_params):
    ...
    beta1 = createSharedDataset(float(search_params['beta1'])) if "beta1" in search_params else createSharedDataset(0.9) 
    beta2=createSharedDataset(float(search_params['beta1'])) if "beta2" in search_params else createSharedDataset(0.999)
    epsilon=createSharedDataset(float(search_params['epsilon'])) if "epsilon" in search_params else createSharedDataset(1e-8)
    ...
    return(method, learning_rate, beta1, beta2, epsilon, lr_decay, lr_growth, eval_step, max_iter, stop_threshold, patience, patience_incr, rand_seed)
...
def trainCoxMlp(x_train, ytime_train, ystatus_train, model_params = dict(), search_params = dict(), verbose=False):
    ...
    method, learning_rate, beta1, beta2, epsilon, lr_decay, lr_growth, eval_step, max_iter, stop_threshold, patience, patience_incr, rand_seed = defineSearchParams(search_params)
    ...
    index_train = ytime_train.argsort()
    x_train = x_train[index_train]
    ytime_train = ytime_train[index_train]
    ystatus_train = ystatus_train[index_train]
    ...
    cost = (
        model.negative_log_likelihood(train_ystatus)
        + L2_reg * model.L2_sqr
    )
    ...
    if method == "momentum":
        ...
        updates = adam(cost, model.params, learning_rate, beta1, beta2, epsilon)
        print("Using adam")
...
def CVLoglikelihood(model, x_full, ytime_full, ystatus_full, x_train, ytime_train, ystatus_train):
    ...
    index_full = ytime_full.argsort()
    x_full = x_full[index_full]
    ystatus_full = ystatus_full[index_full]
    ytime_full = ytime_full[index_full]
    
    theta = model.predictNewData(x_full)
    exp_theta = numpy.exp(theta)
    #newer version of loglik
    #rev(cumsum(rev(theta)))
    
    PL_full = numpy.sum((theta - numpy.log(numpy.flipud(numpy.cumsum(numpy.flipud(exp_theta))))) * ystatus_full)
    ...
    PL_train = numpy.sum((theta - numpy.log(numpy.flipud(numpy.cumsum(numpy.flipud(exp_theta))))) * ystatus_train)
...
def varImportance(model, x_train, ytime_train, ystatus_train):
    ...
    for k in range(x_train.shape[1]):
        ...
        exp_theta = numpy.exp(theta)
        PL_mod[k] = numpy.sum((theta - numpy.log(numpy.flipud(numpy.cumsum(numpy.flipud(exp_theta))))) * ystatus_train)
        
    return(PL_train - PL_mod)   


def permutationImportance(model, times, x_train, ytime_train, ystatus_train):
    N_train = ytime_train.shape[0]
    index_train = ytime_train.argsort()
    x_train = x_train[index_train]
    ystatus_train = ystatus_train[index_train]
    ...
    if times == 1:
    #shuffle x_train (flipud if times=1)
        x_train_s = numpy.flipud(x_train)
        PL_mod = numpy.zeros([x_train.shape[1]])
        for k in range(x_train.shape[1]):
            if (k+1) % 100 == 0:
                print(str(k+1) + "...")
        ...
        PL_mod = numpy.zeros([x_train.shape[1]])
        PL_sum = numpy.zeros([x_train.shape[1]])
        for i in range(times):
            x_train_s = numpy.copy(x_train)
            random.shuffle(x_train_s)  #shuffle x_train
            PL_shuffle = numpy.zeros([x_train.shape[1]])   

            for k in range(x_train.shape[1]):
                if (k+1) % 100 == 0:
                    print(str(k+1) + "...")
            
                xk_shuffle = x_train_s[:,k]
                xk_train = numpy.copy(x_train) #replace x_train[:,k]
                xk_train[:,k] = xk_shuffle
    
                theta = model.predictNewData(xk_train)
                exp_theta = numpy.exp(theta)
                PL_shuffle[k] = numpy.sum((theta - numpy.log(numpy.flipud(numpy.cumsum(numpy.flipud(exp_theta))))) * ystatus_train)
            
            PL_sum = PL_sum + PL_shuffle

        PL_mod = PL_sum/times 
            
    return(numpy.abs(PL_train - PL_mod))

def signOfBeta(model, x_test):
    theta_diff = numpy.zeros((x_test.shape[0], x_test.shape[1]))
    theta = model.predictNewData(x_test)
    for k in range(x_test.shape[1]):
        xk_test_unit = numpy.zeros([x_test.shape[0]])
        xk_test_0 = numpy.copy(x_test) #replace x_train[:,k]
        xk_test_0[:,k] = xk_test_unit
        theta0 = model.predictNewData(xk_test_0)
        theta_diff[:,k] = theta - theta0
        beta_val = numpy.mean(theta_diff, axis=0)
        beta_sign = beta_val/abs(beta_val)
...
def adam(cost, params, learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-8):
    grad = T.grad(cost=cost, wrt=params)
    t_prev = theano.shared(numpy.asarray(0., dtype=theano.config.floatX))
    updates = OrderedDict()
    ...
    for param, g_t in zip(params, grad):
